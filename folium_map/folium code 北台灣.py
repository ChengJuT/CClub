...
def Create_Popup(name, insta_post, website, rating, directions):
...
xmas_map1 = folium.Map(location = [25.0015109,121.2994709],  #參考座標位置,會位於地圖的中心位置
...
marker_cluster = MarkerCluster(name = "Xmas Place_Layer").add_to(xmas_map1)
food = MarkerCluster(name = "Food_Layer").add_to(xmas_map1)
...
for index, row in df.iterrows():
    ...
    popup = Create_Popup(name, insta_post, website, rating, directions)
...
xmas_map1.add_child(marker_cluster)
xmas_map1.add_child(food)

folium.LayerControl().add_to(xmas_map1)

### 存檔
xmas_map1.save('xmas_map1.html')
